...
`ledger/src/compression.rs` exposes the benchmark-facing processing path used to collect public metrics.
...
Use `zchain-bench-dir` against captured validator artifacts and export CSV rows containing ratio, savings, throughput, and round-trip status.
