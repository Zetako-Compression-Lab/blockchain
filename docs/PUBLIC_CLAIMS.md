...
**No public ZChain speed figure is currently claimed from the initial Agave development-build benchmarks.** Those measurements were produced through non-release benchmark commands and are intentionally withheld until release-mode testing is completed on representative hardware.
...
- Any encode/decode MiB/s value derived from the initial development-build benchmark run.
...
- **Release-mode and target-hardware benchmarks are required before publishing current ZChain performance figures.**
...
- measure encode/decode throughput and latency distributions with an appropriate in-memory benchmark path;
- collect shred-shadow measurements on representative ledger traffic;
- publish distributions, not only selected examples, for any future network, storage, or performance claim;
